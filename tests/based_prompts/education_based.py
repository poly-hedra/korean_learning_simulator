...
# [2] vocabulary.json 동적 주입 ← [1] 완료 후
#
# as-is: 참고 어휘 프롬프트 하단에 하드코딩
...
#
#   코드 처리:
#   → 관계 유형이 "낯선 사람"이면 vocabulary["인물"] 함께 주입
#   → vocabulary["전체"] 항상 주입
#
#   LLM 처리 (5번 mission 생성):
#   → vocabulary["전체"] 중 {dialogue_function}과
#      어울리는 단어 조합해서 사용
#
#   검증 후: 어휘 선택이 느슨하면 대화 기능별 매핑 테이블 추가 고려
